...
```mermaid
flowchart TB
  P[Prerequisites:\n1) Digital linkage to LLM\n2) Digital instance of the robot]:::req

  RE[Room environment\n(Unity)]:::node
  AB[Animal behavior\n(Cat)]:::node
  RB[Robot behavior\n+ sensor data]:::node

  subgraph UE[Unity Proposal / Simulation World]
    SE[SIMULATION ENVIRONMENT\n(Unity)\nRoom + Cat + Robot\n(MCP Server integration point)]:::sim
  end

  subgraph VS[Vultr Server]
    BR[Server Bridge / API Layer\n(message routing + formatting)]:::server
    MCP[MCP Server (planned / optional)\nTool calling + structured IO]:::server
  end

  LLM[LLM Reasoning Engine\n(e.g., Gemini)]:::llm
  ROB[Robot Runtime\n(Vector / other robots)]:::robot
  ...
  classDef req fill:#ffd6cc,stroke:#333,stroke-width:1px,color:#111;
  classDef node fill:#ff8a00,stroke:#333,stroke-width:1px,color:#111;
  classDef sim fill:#00a8d6,stroke:#333,stroke-width:1px,color:#111;
  classDef server fill:#ffffff,stroke:#333,stroke-width:1px,color:#111;
  classDef llm fill:#e8f0ff,stroke:#333,stroke-width:1px,color:#111;
  classDef robot fill:#e6ffe8,stroke:#333,stroke-width:1px,color:#111;
